...
function SessionEntry(session) {
    return (
        <li key={session.name} className="py-4 flex">
            <img className="h-10 w-10 rounded-full" src={session.User.UserUploads[0].url} alt="" />
            <img className="h-10 w-10 rounded-full" src={session.SessionUploads[0].url} alt="" />
              <div className="ml-3">
                <p className="text-sm font-medium text-gray-900">{session.name}</p>
                <p className="text-sm font-medium text-gray-900">{session.User.firstname + ' ' + session.User.lastname}</p>
                <p className="text-sm text-gray-500">{session.summarization.join(' ')}</p>
              </div>
            </li>       
      )
}
